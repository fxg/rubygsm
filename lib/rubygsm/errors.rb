...
module Gsm
  class Error < StandardError
    ...
    def to_s
      ...
      return(ERRORS[@type][@code]) if @type && ERRORS[@type] && @code && ERRORS[@type][@code]

      # fall back to something not-so useful
      "Unknown error (unrecognized command?) [type=#{@type}] [code=#{code}]"
    end
  end
  ...
  class WriteError < Error #:nodoc:
    def to_s
      "The modem couldn't be written to. It may have crashed or been unplugged"
    end
  end

  class ReadError < Error #:nodoc:
    def to_s
      "The modem couldn't be read from. It may have crashed or been unplugged"
    end
  end
